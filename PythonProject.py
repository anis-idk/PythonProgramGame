...
while running:
    if current_state == GAME:
    ...
    elif current_state == LOGIN:  #check if current state is login
        for event in pygame.event.get():  #loop through events
            if event.type == pygame.QUIT:
                running = False  #stop the program if the user stop the code or close the window
            elif event.type == pygame.MOUSEBUTTONDOWN:
                input_active = input_box.collidepoint(event.pos) # Check if the user clicked the input box and activated it
            elif event.type == pygame.KEYDOWN: # Handle keyboard input
                if input_active:  # give access to input only if the input box is active (clicked)
                    if event.key == pygame.K_RETURN and input_text.strip():
                        current_player = input_text
                        current_state = MAIN_MENU # Switch to the main menu after the user enter its username and press
                        #enter
                    elif event.key == pygame.K_BACKSPACE:# delete last character of the username when backspace pressed
    ...
    elif current_state == MAIN_MENU: #check if current state is main menu
        stop_music()
        for event in pygame.event.get(): #loop though pygame events
            if event.type == pygame.QUIT: #stop the game if the user closed the window
                running = False
            for i, button in enumerate(main_menu_buttons): #loop through main menu button
                if button.handle_event(event):
                    if i == 0:  # Play game if the button is clicked
                        reset_game() #start the game form 0 every time
                        current_state = GAME
                    elif i == 1:  # Leaderboard   #switch and show to leaderboard screen
                        current_state = LEADERBOARD
                    elif i == 2:  # Quit   #quit the game
                        running = False

        draw_main_menu()

    elif current_state == GAME_OVER: #check if the current state is the game over screen
        stop_music()
        for event in pygame.event.get(): #loop though pygame events
            if event.type == pygame.QUIT: #stop the program when user close the window or stop the code
                running = False
            for i, button in enumerate(game_over_buttons): #loop though game Over screen buttons ("try again & main menu" )
                if button.handle_event(event):
                    if i == 0:  # Try Again
                        reset_game()
                        current_state = GAME
        ...
        for event in pygame.event.get(): #loop through pygame events
            if event.type == pygame.QUIT:  #stop the program if the user close the window or stop the game
                running = False
            for i, button in enumerate(leaderboard_buttons): #loop through leaderboard buttons (main menu & sort )
                if button.handle_event(event):
                    if i == 0:  # Back to Menu
                        current_state = MAIN_MENU
